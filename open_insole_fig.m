function open_insole_fig(app, side)
...
fig_str = ['figure_insole_' lower(side) '_raw'];
fig_pos = [1116         260         671        1085];

% insole figure is saved in the main app app
app.(fig_str) = figure('Pos', fig_pos, 'Tag', fig_str, 'Name', [side ' FSR Insole']);
...
for cnt = 1:8
	...
	h_line = line(t, app.vicon_data.devices.tbl.(fsr_var), ...
...
checkbox_var = [upper(side(1)) lower(side(2:end)) 'InsoleRawCheckBox'];
...
uicontrol(h_fig, ...
		'Style', 'checkbox', ...
		'Tag', 'show_hs_med_chkbx', ...
		'String', {'Show HS'}, ...
		'Units', 'normalized', ...
		'Position', [0.0187,0.74,0.094,0.0166], ...
		'FontSize', 9, ...
		'Value', 0, ...
		'Callback', {@pb_show_insole_events_callback, 'hs_med', app});
uicontrol(h_fig, ...
		'Style', 'edit', ...
...
uicontrol(h_fig, ...
		...
		'String', {'Show TO'}, ...
		'Units', 'normalized', ...
		'Position', [0.0187,0.212,0.094,0.0166], ...
		'FontSize', 9, ...
		'Value', 0, ...
		'Callback', {@pb_show_insole_events_callback, 'to_lat', app});
