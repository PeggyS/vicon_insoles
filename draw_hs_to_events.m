...
% inv hs
for e_cnt = 1:length(app.event_struct.(inv_hs).times)
	t = app.event_struct.(inv_hs).times(e_cnt);
...
% inv to
for e_cnt = 1:length(app.event_struct.(inv_to).times)
	t = app.event_struct.(inv_to).times(e_cnt);
...
% uninv hs
for e_cnt = 1:length(app.event_struct.(uninv_hs).times)
	t = app.event_struct.(uninv_hs).times(e_cnt);
...
% uninv to
for e_cnt = 1:length(app.event_struct.(uninv_to).times)
	t = app.event_struct.(uninv_to).times(e_cnt);
